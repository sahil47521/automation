...
class YouTubeAutomation {
  constructor() {
    ...
    this.quizData = JSON.parse(
      fs.readFileSync(path.join(__dirname, "data/quizData.json"), "utf8"),
    );
  ...
  async init() {
    ...
    if (dbState) {
      console.log(
        `📡 [MongoDB] Loaded state: lastIndex = ${dbState.lastIndex}`,
      );
      this.state = dbState;
    } else {
  ...
  async saveState() {
    ...
    fs.writeFileSync(
      this.stateFile,
      JSON.stringify({ lastIndex: this.state.lastIndex }, null, 2),
    );
  ...
  async syncWithYouTube() {
    ...
    try {
        ...
        for (const activity of response.data.items) {
          if (activity.snippet.type === "upload") {
            ...
            if (match) {
              ...
              console.log(
                `✅ [Recovery] Found last index on YouTube: #${foundIndex}`,
              );
      ...
      console.log(
        "ℹ️ [Recovery] No previous uploads found with ID. Starting from scratch or local state.",
      );
    } catch (err) {
      console.warn(
        "⚠️ [Recovery] Could not sync with YouTube, using local state.",
        err.message,
      );
    }
  }
  ...
  async runSingle(index = null) {
    ...
    try {
      ...
      const thinking = [
        "Sochiye... Sahi jawab kya hai?",
        "Guess kijiye... Time shuru hota hai ab.",
        "Kya aapko iska jawab pata hai? Sochiye...",
        "Tick-tock! Guess the right answer.",
      ];
      ...
      const randomKey =
        eduKeywords[Math.floor(Math.random() * eduKeywords.length)];
      const titleFormats = [
      ...
      if (config.automation.playlists.shorts) {
        await this.uploader.addToPlaylist(
          videoId,
          config.automation.playlists.shorts,
        );
      }
